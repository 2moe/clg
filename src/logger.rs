...
#[wasm_bindgen(js_class = _clgConsoleLogger)]
impl ConsoleLogger {
  ...
  /// ```
  ///
  /// > Within the same thread, the logger can only be initialized once. If you
  /// > choose to initialize in js, do not call `init_logger()` in rust.
  ///
